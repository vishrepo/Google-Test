...
//		FirefoxDriver driver = (FirefoxDriver) WebTesting.launchDriver("https://www.google.com", "firefox");
		...
		// #1: Ensure the search bar is visible before accessing it
		try {
		...
		// #2: Enter "internet speed test" into the search bar and press Return
		driver.findElement(By.name("q")).sendKeys("internet speed test" + Keys.RETURN);

		// #3: Ensure the "RUN SPEED TEST" button is clickable
		try {
			shortWait.until(ExpectedConditions
					.elementToBeClickable(By.id("knowledge-verticals-internetspeedtest__test_button")));
		} catch (TimeoutException ex) {
			fail("\"RUN SPEED TEST\" button not found.");
		}

		// #4: Click the "RUN SPEED TEST" button
		driver.findElement(By.id("knowledge-verticals-internetspeedtest__test_button")).click();

		// #5: Ensure the "CANCEL" button is clickable directly after this
		try {
			shortWait.until(
					ExpectedConditions.elementToBeClickable(By.cssSelector("g-raised-button[jsaction='dArJMd']")));
		} catch (TimeoutException ex) {
			fail("\"CANCEL\" button not found.");
		}

		// #6: Wait for the test to finish (CANCEL button no longer clickable)
		try {
		...
		// #7: Ensure "RETRY" does not appear (failure condition)
		try {
			shortWait.until(ExpectedConditions
					.visibilityOfElementLocated(By.cssSelector("g-raised-button[jsaction='i0JLwd']")));
			fail("Speed test failed to run.");
		} catch (TimeoutException ex) {
			// Expected condition, test passes
		}

		// #8: Ensure "TEST AGAIN" button is visible (success condition)
		try {
			shortWait.until(ExpectedConditions
					.elementToBeClickable(By.xpath("//g-raised-button[@jsaction=\'iyDKIb\']")));
		} catch (TimeoutException ex) {
			fail("Speed test failed to run.");
		}
	}
...
//		FirefoxDriver driver = (FirefoxDriver) WebTesting.launchDriver("https://www.google.com", "firefox");
	ChromeDriver driver = (ChromeDriver) WebTesting.launchDriver("https://www.google.com", "chrome");
		...
		// #9: TODO (enter "internet speed test" into the search bar then press the
		// "Return" key)
		...
		try {
			cancelButton = driver.findElement(By.xpath("//g-raised-button[@jsaction=\'dArJMd\']"));
		} catch (NoSuchElementException ex) {
			fail("\"CANCEL\" button not found.");
		}
...
//		FirefoxDriver driver = (FirefoxDriver) WebTesting.launchDriver("https://www.google.com", "firefox");
	ChromeDriver driver = (ChromeDriver) WebTesting.launchDriver("https://www.google.com", "chrome");
		...
		// #15: TODO (enter "calculator" into the search bar then press the "Return"
		// key)
...
//		}
		...
		// #18: TODO (enter "+" and check whether the value was input into the text
		// field)
		...
		// #19: TODO (enter "-" and check whether the value was input correctly into the
		// text field)
		...
		// #20: TODO (enter "2" and check whether the value was input into the text
		// field)
	...
	public void testCalculatorImplicitly() throws InterruptedException {
	    ...
	    // #29: Ensure the Calculator is visible
	    try {
	    ...
	    // #30: Input "1" and verify it's displayed correctly
	    ...
	    // #31: Input "+" and verify it's displayed correctly
	    ...
	    // #32: Input "-" and verify it's displayed correctly
	    ...
	    // #33: Input "2" and verify it's displayed correctly
	    ...
	    // #34: Press "=" and verify the result
	    driver.findElement(By.xpath("//div[@jsname=\'Pt8tGc\']")).click();
	    try {
	        driver.findElement(By.xpath("//span[@id='cwos' and text()='-1']"));
	    } catch (NoSuchElementException ex) {
	    ...
	    // #35: Press "AC" and verify if cleared
	    driver.findElement(By.xpath("//div[@jsname=\'SLn8gc\']")).click();
	    try {
	    ...
	    // Optional: Add a brief sleep or wait if necessary for any background processing
	    try {
